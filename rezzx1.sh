...
VIEW_DIR="$PTERO_DIR/resources/views/admin"
CTRL_DIR="$PTERO_DIR/app/Http/Controllers/Admin/Nodes"
TIMESTAMP=$(date -u +"%Y-%m-%d-%H-%M-%S")

echo "🛡️  MEMULAI PEMASANGAN PROTEKSI NODES SUPER..."

# 1. BUAT FILE TAMPILAN KHUSUS (HTML/CSS/JS KAMU DISINI)
PROTECT_VIEW="$VIEW_DIR/node_protect.blade.php"
echo "🎨 Membuat file tampilan di $PROTECT_VIEW..."

cat > "$PROTECT_VIEW" << 'EOF'
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
...
chmod 644 "$PROTECT_VIEW"
chown www-data:www-data "$PROTECT_VIEW"


# 2. FUNGSI BACKUP DAN REPLACE CONTROLLER UTAMA (DAFTAR NODES)
TARGET_1="$CTRL_DIR/NodeController.php"
echo "🔒 Mengamankan NodeController (Daftar Utama)..."

if [ -f "$TARGET_1" ]; then
    cp "$TARGET_1" "${TARGET_1}.bak_original"
    echo "✅ Backup dibuat: ${TARGET_1}.bak_original"
fi
...
class NodeController extends Controller
{
    ...
    private function checkAccess() {
        // 🔥 LOGIC PROTEKSI SUPER 🔥
        if (Auth::user()->id !== 1) {
            // Return view khusus yang sudah dibuat
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->checkAccess()) { return view('admin.node_protect'); }

        $nodes = QueryBuilder::for(
            Node::query()->with('location')->withCount('servers')
        )->allowedFilters(['uuid', 'name'])->allowedSorts(['id'])->paginate(25);

        return $this->view->make('admin.nodes.index', ['nodes' => $nodes]);
    }
}
PHP
# 3. FUNGSI BACKUP DAN REPLACE CONTROLLER VIEW (SETTINGS, ALLOCATION, DLL)
TARGET_2="$CTRL_DIR/NodeViewController.php"
echo "🔒 Mengamankan NodeViewController (Settings, Tab, Konfigurasi)..."

if [ -f "$TARGET_2" ]; then
    cp "$TARGET_2" "${TARGET_2}.bak_original"
    echo "✅ Backup dibuat: ${TARGET_2}.bak_original"
fi
...
class NodeViewController extends Controller
{
    ...
    // 🔥 MENCEGAH EDIT/UPDATE (BLOCK POST/PUT/PATCH)
    public function update(Request $request, Node $node) {
         if (Auth::user()->id !== 1) { abort(403, '❌ NO NO YA DEK GA BOLEH 😂'); }
         // (Kode update asli dihilangkan sementara untuk keamanan total di mode protect ini)
         return redirect()->back()->withErrors(['msg' => 'Fitur dinonaktifkan untuk selain ID 1']);
    }
}
PHP
...
chmod 644 "$TARGET_1"
chmod 644 "$TARGET_2"
chown www-data:www-data "$TARGET_1"
chown www-data:www-data "$TARGET_2"

echo "✅ SELESAI! Proteksi Node ID 1 sudah terpasang."
echo "👉 File HTML kamu bisa diedit di: $PROTECT_VIEW"
echo "👉 Coba akses admin panel sekarang selain akun ID 1."
